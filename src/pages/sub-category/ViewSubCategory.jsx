...
// View Sub Category page — displays sub categories in a table with parent category, image, order, status, and edit action
export default function ViewSubCategory() {
    ...
    if (selectedRecord.length > 0) {
      try {
        const result = await axios.put(
          `${import.meta.env.VITE_SERVER_URL}api/backend/sub-categories/toggle-status`,
          {
            id: selectedRecord,
          },
        );
        ...
        buttons: [
          [
            ...
            async function (instance, toast) {
              ...
              try {
                const result = await axios.post(
                  `${import.meta.env.VITE_SERVER_URL}api/backend/sub-categories/delete`,
                  {
                    id: selectedRecord,
                  },
                );
  ...
  const applyFilter = (e) => {
    ...
    const obj = {
      name: form.subCategoryName.value,
  ...
  useEffect(() => {
    const fetchSubCategories = async () => {
      try {
        const result = await axios.post(
          `${import.meta.env.VITE_SERVER_URL}api/backend/sub-categories/view`,
          {
            page: currentPage,
  ...
  return (
    <>
      <div className="form-wrapper mt-10 mx-5 rounded-xl overflow-auto shadow-2xl">
        ...
        {/* Sub Category table — short content columns are center-aligned, text columns are left-aligned */}
                ...
                <th className="py-3 px-4 text-center text-nowrap">
                  <input
                    ...
                    checked={
                      ...
                      selectedRecord.length === subCategories.length
                    }
                    onChange={() => {
                      if (selectedRecord.length === subCategories.length) {
                        setSelectedRecord([]);
                      } else {
                        setSelectedRecord(subCategories.map((sub) => sub._id));
                      }
                    }}
                  />
                  Select
                ...
                subCategories.map((element, index) => {
                  return (
                    <tr className="align-middle border-b" key={index}>
                      ...
                      {/* Make sure "element.parent_category_id?.name" matches what your backend actually populates for the parent category */}
                      <td className="py-3 px-4 text-nowrap">
                        {element.parent_category_id?.name || "N/A"}
                      </td>
                      ...
                      <td className="pt-8 px-4 flex items-center justify-center text-yellow-400">
                        <Link to={`/sub-category/update/${element._id}`}>
                          <SquarePen />
                        </Link>
                      </td>
                    </tr>
