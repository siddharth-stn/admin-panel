...
export default function ViewProduct() {
  ...
  useEffect(() => {
    async function fetchCategories() {
      try {
        const result = await axios.post(`${BASE}api/backend/products/parent-category`);
        if (result.data._status) setCategories(result.data._data || []);
      } catch (error) {
  ...
  const changeStatus = async () => {
    if (selectedRecord.length > 0) {
      try {
        const result = await axios.put(`${BASE}api/backend/products/toggle-status`, {
          id: selectedRecord,
        });
        if (result.data._status) {
          ...
          iziToast.error({ title: "Failed", message: result.data._message, position: "topRight" });
        }
      } catch (error) {
        iziToast.error({ title: "Failed", message: "Something went wrong!", position: "topRight" });
        console.log(error);
      }
    }
  };
        ...
        buttons: [
          [
            ...
            async function (instance, toast) {
              try {
                const result = await axios.post(`${BASE}api/backend/products/delete`, {
                  id: selectedRecord,
                });
                if (result.data._status) {
                  iziToast.success({ title: "Success", message: result.data._message, position: "topRight" });
          ...
          [
            ...
            function (instance, toast) {
              iziToast.info({ title: "Cancelled", message: "Delete action cancelled", position: "topRight" });
              instance.hide({ transitionOut: "fadeOut" }, toast);
            },
          ],
        ],
  ...
  const productTypeLabel = (type) => {
    const map = { 1: "Featured", 2: "On Sale", 3: "New Arrivals" };
    return map[type] || "-";
  };
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const result = await axios.post(`${BASE}api/backend/products/view`, {
          page: currentPage,
    ...
    <>
      <div className="form-wrapper mt-10 mx-5 rounded-xl overflow-auto shadow-2xl">
          ...
          <div className="filterForm mb-4 w-full p-2 border-2 border-black relative">
            ...
            <form className="flex flex-col gap-5">
              <div className="flex gap-6">
                <label htmlFor="productName" className="flex flex-col gap-2">
        ...
        <div className="form-header border border-gray-200 dark:border-gray-700 py-5 px-6 rounded text-white bg-blue-600 dark:bg-blue-400 dark:text-white hover:bg-blue-700 dark:hover:bg-blue-500">
          <div className="flex">
            <h3 className="text-2xl font-bold dark:text-white">View Products</h3>
              ...
              <tr>
                <th className="py-3 px-4 text-center text-nowrap">
                  <input
                    type="checkbox"
                    className="mr-2"
                    checked={products.length > 0 && selectedRecord.length === products.length}
                    onChange={() => {
                      if (selectedRecord.length === products.length) {
                        setSelectedRecord([]);
                      } else {
                        setSelectedRecord(products.map((p) => p._id));
                      }
                    }}
                  />
                  Select
                ...
                <th className="py-3 px-4 text-left text-nowrap">Category</th>
                <th className="py-3 px-4 text-center text-nowrap">Type</th>
                <th className="py-3 px-4 text-center text-nowrap">Sale Price</th>
                <th className="py-3 px-4 text-center text-nowrap">Actual Price</th>
            ...
            <tbody>
              {products.length > 0 ? (
                products.map((element, index) => (
                  <tr className="align-middle border-b" key={element._id}>
                    <td className="py-3 px-4 text-center">
                      <input
                        type="checkbox"
                        onClick={() => singleCheckSelect(element._id)}
                        checked={selectedRecord.includes(element._id)}
                      />
                    </td>
                    ...
                    <td className="py-3 px-4 text-nowrap">
                      {element.parent_category_id?.name || "-"}
                    </td>
                    <td className="py-3 px-4 text-center text-nowrap">
                      {productTypeLabel(element.product_type)}
                    </td>
                    <td className="py-3 px-4 text-center">{element.sale_price}</td>
                    <td className="py-3 px-4 text-center">{element.actual_price}</td>
                    ...
                    <td className="py-3 px-4 text-yellow-400">
                      <Link to={`/product/update/${element._id}`} className="flex justify-center">
                        <SquarePen />
                      </Link>
                    </td>
                  </tr>
                ...
                <tr className="border-b">
                  <td className="py-3 px-4 text-center" colSpan={11}>
                    No products found
                  </td>
                </tr>
              )}
