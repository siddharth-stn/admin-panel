...
export default function AddProduct() {
      ...
      if (
        ...
        try {
          const result = await axios.post(
            `${BASE}api/backend/products/sub-category`,
            {
  ...
  useEffect(() => {
    async function fetchSubSubCategories() {
      if (formData.sub_category_id && formData.sub_category_id !== "default") {
        try {
          const result = await axios.post(
            `${BASE}api/backend/products/sub-sub-category`,
            {
  ...
  useEffect(() => {
    if (productId) {
      async function getProductDetails() {
        try {
          const result = await axios.post(
            `${BASE}api/backend/products/details/${productId}`,
          );
          if (result.data._status) {
            ...
            setFormData({
              parent_category_id:
                d.parent_category_id?._id || d.parent_category_id || "default",
              sub_category_id:
                d.sub_category_id?._id || d.sub_category_id || "default",
              sub_sub_category_id:
                d.sub_sub_category_id?._id ||
                d.sub_sub_category_id ||
                "default",
              name: d.name || "",
              product_type: d.product_type?.toString() || "default",
              best_selling: d.best_selling?.toString() || "default",
              material_ids: d.material_ids
                ? d.material_ids.map((m) => m._id || m)
                : [],
            ...
            if (d.parent_category_id?._id || d.parent_category_id) {
              const parentId =
                d.parent_category_id?._id || d.parent_category_id;
            ...
            if (d.sub_category_id?._id || d.sub_category_id) {
              ...
              if (subSubRes.data._status)
                setSubSubCategories(subSubRes.data._data || []);
            }
          }
        } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      formData.sub_sub_category_id === "default"
    )
    ...
    if (formData.color_ids.length === 0)
      errors.color_ids = "Select at least one color";
    ...
    if (!formData.estimated_delivery.trim())
      errors.estimated_delivery = "Estimated delivery is required";
  ...
  const inputClass =
    "border border-gray-300 rounded px-3 py-1 focus:outline-none focus:ring-2 focus:ring-gray-500";
        ...
        <form onSubmit={handleSubmit} className="relative">
          <div className="formContent flex flex-col px-5 py-4 text-[18px] gap-5 pb-20">
            ...
            <div className="flex gap-5">
              ...
              <div className="flex flex-col gap-2 flex-1">
                <label htmlFor="sub_sub_category_id">
            ...
            <div className="flex gap-5">
              <div className="flex flex-col gap-2 flex-1">
                <label htmlFor="material_ids">
                  Materials (hold Ctrl to select multiple)
                </label>
                <select
                  multiple
                  id="material_ids"
            ...
            <div className="flex gap-10">
              ...
              <label
                ...
                className="image-wrapper flex flex-col gap-3 cursor-pointer"
              >
                ...
                <div className="flex gap-3 flex-wrap">
                  {multiImageUrls.length > 0 ? (
                    multiImageUrls.map((url, idx) => (
                      <figure
                        key={idx}
                        className="bg-gray-300 h-32 w-32 rounded-xl p-1"
                      >
                        <img
                          className="w-full h-full object-cover rounded-xl"
                          src={getImageSrc(url)}
